...
SELECT CE.EventOrganiserId, CE.EventName, C.CityName FROM Cities C JOIN CharityEvent CE ON C.CityId=CE.CityId
...
--8.Post Feed
(
SELECT P.PostId, UD.ProfileImage, UD.UserName, P.LocationName, P.ImageURL, CAST(P.Description AS NVARCHAR) AS 'Post Description', P.HelpRequiredCount, 
...
WHERE P.PincodeId = 6 AND P.IsClosed = 0--@User'sPincodeId
)
UNION
(
SELECT P.PostId, UD.ProfileImage, UD.UserName, P.LocationName, P.ImageURL, CAST(P.Description AS NVARCHAR) AS 'Post Description', P.HelpRequiredCount, 
...
LEFT JOIN (SELECT PostId, COUNT(UserId) AS 'UrgencyCount' FROM Urgency GROUP BY PostId) UC ON P.PostId = UC.PostId
LEFT JOIN (SELECT PostId, COUNT(SpamId) AS 'SpamCount' FROM Spam GROUP BY PostId) SC ON P.PostId = SC.PostId
WHERE P.CityId = 1 AND P.IsClosed = 0--@User'sCityId
)
UNION
(
SELECT P.PostId, UD.ProfileImage, UD.UserName, P.LocationName, P.ImageURL, CAST(P.Description AS NVARCHAR) AS 'Post Description', P.HelpRequiredCount, 
...
LEFT JOIN (SELECT PostId, COUNT(UserId) AS 'UrgencyCount' FROM Urgency GROUP BY PostId) UC ON P.PostId = UC.PostId
LEFT JOIN (SELECT PostId, COUNT(SpamId) AS 'SpamCount' FROM Spam GROUP BY PostId) SC ON P.PostId = SC.PostId
WHERE P.StateId = 1 AND P.IsClosed = 0--@User'sStateId
)
UNION
(
SELECT P.PostId, UD.ProfileImage, UD.UserName, P.LocationName, P.ImageURL, CAST(P.Description AS NVARCHAR) AS 'Post Description', P.HelpRequiredCount, 
...
WHERE C.CityName = 'Ahmedabad' AND P.IsClosed = 0
